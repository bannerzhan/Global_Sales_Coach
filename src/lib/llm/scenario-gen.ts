...
const FALLBACK_OPENING_ZH = "您好，我在网上看到贵公司的产品目录，想了解一下报价。";
const FALLBACK_OPENING_EN = "Hi, I came across your product catalog online and wanted to ask about pricing.";
const fallbackOpening = (locale?: string | null) =>
  locale === "en" ? FALLBACK_OPENING_EN : FALLBACK_OPENING_ZH;
  ...
  const result = await runContract<ScenarioGenResult>(
    {
      ...
      fallback: () => ({
        ...
        openingLine: fallbackOpening(locale),
      }),
      system:
        ...
        `   ${skillList}\n` +
        (lang === "en"
          ? "5. openingLine is the buyer's first message — in English, natural, like a real inbound inquiry\n" +
            "   scenario title in English"
          : "5. openingLine 是 AI 客户的第一句话，用中文、口语化、贴近真实询盘\n" +
            "   场景标题用中文") + focusInstruction,
    },
    [
      {
        role: "user",
        content:
          ...
          (lang === "en" ? "Generate an English practice scenario." : "请生成一个中文演练场景。") +
          (focusDef
